...
// <button type="button" id = "remove_form" class="btn btn-danger">Remove</button>\
	function format_cp_form(cps_group,key,cp){
		...
		$('<form class = "cp_form">\
			<img src="{{"images/delete.png"}}" id = "remove_form"/>\
			  ...
			  <div class="row">\
                <br>\
			    <div class="col">\
			      <textarea style ="width: 100%;height: 600px; " id = "__spots" name = "spots"></textarea>\
			    </div>\
			  </div>\
			  <hr/>\
			</form>');
        ...
        let raw_cp_data = cp.spots;
        let spots = "";
        for(let key in raw_cp_data){
            const pattern = new RegExp(/[0-9]{2}:/);
            let spot = raw_cp_data[key];
            // console.log(spot);
            if(!pattern.test(spot.substring(0,3))){
                spots += spot.substring(0,3)+"\n";
                spots += spot.substring(3).replace(/\s/g, '    ')+"\n";
            }else{
                spots += spot.replace(/\s/g, '    ')+"\n";
            }
        }
        
        $container.find("#__spots").val(spots);
		return $container;
	}
	...
	function arranged_data(){
		...
		for (var i = 0; i < forms.length; i++) { // form
			...
			spot = spot.split(/\s/g).filter(data => {
                return data != "";
            });
